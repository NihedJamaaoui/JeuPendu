...
public class StoreController implements Initializable {
    ...
    private String lastSelectedTabType = "KEYBOARD_SKIN";
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        storeTabs.getSelectionModel().selectedItemProperty().addListener((obs, oldTab, newTab) -> {
            if (newTab != null) {
                lastSelectedTabType = (String) newTab.getUserData();
            }
        });
    }
    ...
    private void populateStoreTabs() {
        ...
        Map<String, String> tabTypes = Map.of(
                "KEYBOARD_SKIN", "Thèmes de clavier",
                "BACKGROUND", "Fonds d'écran",
                "AVATAR", "Avatars"
        );

        Tab tabToSelect = null; // on garde une référence au tab à sélectionner

        for (Map.Entry<String, String> entry : tabTypes.entrySet()) {
            ...
            if (!itemsOfType.isEmpty()) {
                ...
                tab.setUserData(type); // très important
                tab.setContent(createItemsGrid(itemsOfType));
                storeTabs.getTabs().add(tab);

                // si ce tab correspond au type précédemment sélectionné
                if (type.equals(lastSelectedTabType)) {
                    tabToSelect = tab;
                }
            }
        }

        // Sélection après avoir ajouté tous les tabs
        if (tabToSelect != null) {
            storeTabs.getSelectionModel().select(tabToSelect);
        } else if (!storeTabs.getTabs().isEmpty()) {
            // fallback si lastSelectedTabType est invalide
            storeTabs.getSelectionModel().selectFirst();
            lastSelectedTabType = (String) storeTabs.getSelectionModel().getSelectedItem().getUserData();
        }
    }
    ...
    private void purchaseItem(StoreItem item) {
        if (currentUser.getCoins() >= item.getPrice()) {
            // Store the current tab before refreshing
            Tab selectedTab = storeTabs.getSelectionModel().getSelectedItem();
            if (selectedTab != null && selectedTab.getUserData() instanceof String) {
                lastSelectedTabType = (String) selectedTab.getUserData();
            }
            ...
            // Refresh while maintaining the current tab
            UserService.updateUserStats(currentUser);
            populateStoreTabs();
